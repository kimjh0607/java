...
public class FileClassRun {
	public static void main(String[] args) throws IOException {
		String path = "D:\\workspace01\\javastudy\\src\\study17\\files";
		File dir = new File(path);
		if(!dir.exists()) {//!없음
			dir.mkdir();//만듦
		}
		System.out.println(dir.exists());//있음
		
		File file = new File(path+"\\TestFile.txt");
		if(!file.exists()) {
			file.createNewFile();//파일만드는 메서드
		}
		System.out.println(file.exists());
		
		File file2 = new File(path+"D:\\workspace01\\javastudy\\src\\study17\\files\\TestFile.txt");
		File file3 = new File(path+"D:/workspace01/javastudy/src/study17/files/TestFile.txt");
		File file4 = new File(path+"D:"+File.separator+"workspace01"+File.separator+"javastudy"+File.separator+"src"
		+File.separator+"study17"+File.separator+"files"+File.separator+"TestFile.txt");
		
		System.out.println(file2.exists()+" "+file2.getAbsolutePath());//절대경로메서드
